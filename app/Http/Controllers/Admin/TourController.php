...
class TourController extends Controller
{
    ...
    /**
     * Sync tour model fields from final_json data
     * - when db_sync is true: force-sync all known map fields
     * - otherwise: keep existing behavior using diff paths
     * @param Tour $tour
     * @param array $finalJson
     * @param array $diffJson
     * @param bool $forceSync
     * @return void
     */
    private function syncTourFieldsFromJson(Tour $tour, array $finalJson, array $diffJson = [], bool $forceSync = false): void
    {
    ...
    public function updateTourJson(Tour $tour, Request $request): JsonResponse|RedirectResponse
    {
        ...
        try {
            ...
            $this->syncTourFieldsFromJson($tour, $finalJson, $diffJson, true);
    ...
    public function uploadJsonFile(Request $request, Tour $tour)
    {
        $request->validate([
            ...
            'DB_sync' => 'nullable|boolean,default:false'
        ]);
        ...
        $this->syncTourFieldsFromJson($tour, $json, [], false);
        // save tour updates once after syncing fields
        $tour->save();
