...
<div class="row">
    ...
    <div class="col-xl-9 col-lg-8 ">
        ...
        <div class="card">
            ...
            <div class="card-body">
                @php
                    $isEdit = isset($scheduler);
                @endphp
                <form id="scheduler-form" method="POST" action="{{ $isEdit ? route('schedulers.update', $scheduler) : route('schedulers.store') }}">
                    @csrf
                    @if($isEdit)
                        @method('PUT')
                    @endif
                    ...
                    <div class="col-lg-6">
                            <div class="mb-3">
                                ...
                                <input type="text" id="agent-name" name="firstname" value="{{ old('firstname', $scheduler->firstname ?? '') }}" class="form-control" placeholder="Full Name" required>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="mb-3">
                                <label for="agent-email" class="form-label">Agent Email</label>
                                <input type="email" id="agent-email" name="email" value="{{ old('email', $scheduler->email ?? '') }}" class="form-control" placeholder="Enter Email" required>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="mb-3">
                                <label for="agent-number" class="form-label">Agent Number</label>
                                <input type="text" id="agent-number" name="mobile" value="{{ old('mobile', $scheduler->mobile ?? '') }}" class="form-control" placeholder="Enter Number" required>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="mb-3">
        ...
        <div class="mb-3 rounded">
            <div class="row justify-content-end g-2">
                <div class="col-lg-2">
                    <button formmethod="post" type="submit" form="scheduler-form" class="btn btn-outline-primary w-100">{{ $isEdit ? 'Update Agent' : 'Create Agent' }}</button>
                </div>
                <div class="col-lg-2">
                    <a href="{{ route('schedulers.index') }}" class="btn btn-danger w-100">Cancel</a>
                </div>
            </div>
        </div>
    </div>
