@extends('admin.layouts.vertical', ['title' => 'Edit User', 'subTitle' => 'System'])
                ...
                <form method="POST" action="{{ route('admin.users.update', $user) }}" class="needs-validation" novalidate>
                            ...
                            @forelse($roles as $role)
                                <div class="col-lg-4">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="roles[]" value="{{ $role->name }}" id="role_{{ $role->id }}" {{ $user->roles->contains('id',$role->id) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="alert alert-info mb-3">
                            You do not have permission to manage user roles. Existing roles are shown for reference only.
                        </div>
                        <div class="row g-2">
                            @forelse($user->roles as $role)
                                <div class="col-lg-4">
                                    <span class="badge bg-soft-primary text-primary w-100 text-center">{{ $role->name }}</span>
                                </div>
                            @empty
                                <div class="col-12">
                                    <p class="text-muted mb-0">This user currently has no roles.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
