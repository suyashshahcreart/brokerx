...
@section('script')
    ...
    <script>
        ...
        // Update top update button label based on active tab and submit active tab's form
        (function() {
            const topBtn = document.getElementById('top-update-btn');
            if (!topBtn) return;

            const labelMap = {
                'booking-tab': 'Update Booking',
                'tour-tab': 'Update Tour',
                'seo-tab': 'Update SEO',
                'quick-actions-tab': 'Quick Actions',
                'json-tab': 'Update JSON'
            };

            function setTopButtonLabel(tabId) {
                const label = labelMap[tabId] || 'Update';
                topBtn.innerHTML = '<i class="ri-save-line me-1"></i> ' + label;
            }

            // Initial label from currently visible pane (more robust than relying on the nav button active class)
            const activePane = document.querySelector('.tab-pane.show.active') || document.querySelector('.tab-pane.active');
            if (activePane) {
                const paneId = activePane.id; // e.g. booking-pane
                const paneTabTrigger = document.querySelector(`button[data-bs-target="#${paneId}"], button[aria-controls="${paneId}"]`);
                if (paneTabTrigger) setTopButtonLabel(paneTabTrigger.id);
                else setTopButtonLabel('booking-tab');
            } else {
                setTopButtonLabel('booking-tab');
            }

            // Listen for tab changes
            const tabTriggers = document.querySelectorAll('button[data-bs-toggle="tab"]');
            tabTriggers.forEach(tt => tt.addEventListener('shown.bs.tab', (e) => {
                setTopButtonLabel(e.target.id);
            }));

            // When top button is clicked, try to submit the visible form inside the active pane
            topBtn.addEventListener('click', function() {
                // Prefer the currently visible pane
                const visiblePane = document.querySelector('.tab-pane.show.active') || document.querySelector('.tab-pane.active') || document.querySelector('#booking-pane');
                if (!visiblePane) return;

                // If there is a form inside this pane, trigger its submit handlers
                const form = visiblePane.querySelector('form');
                if (form) {
                    // Prefer clicking the form's submit button so attached handlers run
                    ...
                    // Fallback: dispatch a submit event so listeners on the form run
                    const ev = new Event('submit', { bubbles: true, cancelable: true });
                    form.dispatchEvent(ev);
                    // If not prevented, call native submit as last resort
                    if (!ev.defaultPrevented) form.submit();
                    return;
                }

                // Otherwise, try to find any submit button in the pane and click it
                const submitBtn = visiblePane.querySelector('button[type="submit"], input[type="submit"]');
                if (submitBtn) {
                    submitBtn.click();
                }
            });
