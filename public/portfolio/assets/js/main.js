...
if (viewerContainers.length > 0) {
  ...
  Promise.all([
    ...
    if (sliderElement && typeof Swiper !== 'undefined') {
      ...
      const panoramas = ['assets/images/babylon/original/360-1.jpg', 'assets/images/sarhaddairy/360-1.jpg', 'assets/images/portfolio/nautanky-360.jpg'];
      const viewers = [];
      const viewerAutos = [];
...
// Initialize Company Logo Slider
document.addEventListener('DOMContentLoaded', function() {
  const companyLogoSlider = document.querySelector('.company-logo-slider');
  if (companyLogoSlider && typeof Swiper !== 'undefined') {
    new Swiper(companyLogoSlider, {
      slidesPerView: 'auto',
      spaceBetween: 30,
      loop: true,
      autoplay: {
...
if (typeof Swiper !== 'undefined') {
  ...
  console.warn('Swiper library not loaded. Testimonials slider will not work.');
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    // Initialize intl-tel-input for phone number with country code
    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      // Validate form - check required fields (address and requirement are optional)
      ...
      if (iti) {
        ...
        if (!iti.isValidNumber()) {
          showMessage('Please enter a valid phone number with country code.', 'danger');
          hideLoader();
          return;
        }
        
        // Get full phone number with country code in international format
        formData.contactPerson = iti.getNumber();
      } else {
        ...
        formData.contactPerson = phoneValue;
      }
...
// Mobile Menu Slider Functionality
document.addEventListener('DOMContentLoaded', function() {
  ...
  function openMobileMenu() {
    ...
    if (mobileMenuSlider) mobileMenuSlider.classList.add('active');
  ...
  function closeMobileMenu() {
    ...
    if (mobileMenuSlider) mobileMenuSlider.classList.remove('active');
  ...
  if (mobileMenuToggle) {
    mobileMenuToggle.addEventListener('click', function(e) {
      e.stopPropagation();
      if (mobileMenuSlider && mobileMenuSlider.classList.contains('active')) {
        closeMobileMenu();
      } else {
        openMobileMenu();
      }
  ...
  mobileMenuLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      if (!this.classList.contains('mobile-menu-link-with-arrow')) {
...
// Navbar Scroll Effect - Add overlay transparency on scroll
document.addEventListener('DOMContentLoaded', function() {
  const navbar = document.querySelector('.navbar-overlay');
  const sliderContainer = document.querySelector('.slider-container');
  
  if (navbar && sliderContainer) {
    function handleScroll() {
      const sliderBottom = sliderContainer.offsetTop + sliderContainer.offsetHeight;
